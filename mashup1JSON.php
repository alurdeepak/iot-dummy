...
//    $GET_M2M_DATA="SELECT t2.prop_name, t1.prop_value,t1.uploaded_on FROM dat_m2m_data t1, dat_properties t2, dat_services t3 WHERE t1.prop_id_fk=t2.id AND t2.service_id_fk=t3.id AND t2.id=";


    $sql=$GET_M2M_DATA . $_REQUEST['pid'] . " order by t1.uploaded_on desc limit 20" ;
...
while($row = mysql_fetch_array($result)){
    ...
    if($count==0){
    //echo "date " . $row['uploaded_on'] . " " . $row['prop_name'] . " " . $row['prop_value'] . "<br>"; 
   $str1="{\"cols\": [{\"id\":\"" . $count1 ."\",\"label\":\"Date\",\"pattern\":\"\",\"type\":\"string\"}";
    $str1=$str1. ",{\"id\":\"" . $count1. "\",\"label\":\"" . $row['prop_name'] . "\",\"pattern\":\"\",\"type\":\"number\"}"; 
      ...
      if($count==$row_count){
$str1=$str1 ."{\"c\":[{\"v\":\"". $row['uploaded_on'] . "\",\"f\":null},{\"v\":\"" . $row['prop_value']. "\",\"f\":null}]}";
    }else{
$str1=$str1 ."{\"c\":[{\"v\":\"". $row['uploaded_on'] . "\",\"f\":null},{\"v\":\"" . $row['prop_value']. "\",\"f\":null}]},";
